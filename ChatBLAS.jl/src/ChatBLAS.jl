...
function sswap()
  ...
  if ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "openmp" 
    prompt = "Only respond with code as plain text without code block syntax around it. Give me a function code only that computes the swap of two vectors x and y. Vectors are length n, use C and OpenMP to compute in parallel, use next function name and parameters void chatblas_sswap(int n, float *x, float *y) {. Include the next line at the beginning #include \"chatblas_openmp.h\""
  elseif ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "cuda" 
    prompt = "Only respond with code as plain text without code block syntax around it. Give me a kernel and a function that calls the kernel only that computes the swap of two vectors x and y. Do not give a main function. Vectors are length n, use C and CUDA to compute in parallel, allocate and free the GPU vectors and make the CPU - GPU memory transfers in the function. The size of blocks of threads and the number of blocks must be defined. Use next function name and parameters for the kernel __global__ void sswap_kernel(int n, float *x, float *y) { and the next function name and parameters for the function void chatblas_sswap(int n, float *x, float *y) {. Include the next line at the beginning of the code #include \"chatblas_cuda.h\"."
  elseif ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "hip" 
...
function sdot()
  ...
  if ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "openmp" 
    prompt = "Only respond with code as plain text without code block syntax around it. Give me a function code only that computes and returns the dot product of two vectors x and y. Vectors are length n, use C and OpenMP to compute in parallel, use next function name and parameters float chatblas_sdot(int n, float *x, float *y) {. Include the next line at the beginning #include \"chatblas_openmp.h\""
  elseif ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "cuda" 
...
function sdsdot()
  ...
  if ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "openmp" 
    prompt = "Only respond with code as plain text without code block syntax around it. Give me a function code that computes and returns the dot product of two vectors x and y. The final result must add the scalar b which is passed as argument. The accomulation is computed in double precision, so the elements of the vector X and Y must be casted to double before the computation. Vectors are length n, use C and OpenMP to compute in parallel, use next function name and parameters float chatblas_sdsdot(int n, float b, float *x, float *y) {. Include the next line at the beginning #include \"chatblas_openmp.h\""
  elseif ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "cuda" 
    prompt = "Only respond with code as plain text without code block syntax around it. Give me a kernel and a function that calls the kernel only that computes and return the dot product of two vectors x and y. The final result must add the scalar b which is passed as argument. The accomulation is computed in double precision, so the elements of the vector X and Y must be casted to double before the computation.  Do not give a main function. The accomulation is computed in double precision. Vectors are length n, use C and CUDA to compute in parallel, allocate and free the GPU vectors and make the CPU - GPU memory transfers in the function. The size of blocks of threads and the number of blocks must be defined. Use shared memory to accelerate the computation. Minimize the use of atomic operations. Use next function name and parameters for the kernel __global__ void sdsdot_kernel( int n, float b, float *x, float *y, float *res ) { where the result of computing the dot product of the vector x and y plus b is returned in the pointer res and the next function name and parameters for the function float chatblas_sdsdot( int n, float b, float *x, float *y) {. Include the next line at the beginning of the code #include \"chatblas_cuda.h\"."
  elseif ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "hip" 
...
function snrm2()
  ...
  if ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "openmp" 
    prompt ="Only respond with code as plain text without code block syntax around it. Give me a function code only that computes and returns the Euclidean norm of a vector x. Vectors are length n, use C and OpenMP to compute in parallel, use next function name and parameters float chatblas_snrm2(int n, float *x) {. Include the next line at the beginning #include \"chatblas_openmp.h\""
  elseif ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "cuda" 
    prompt = "Only respond with code as plain text without code block syntax around it. Give me a kernel and a function that calls the kernel only that computes and return the Euclidean norm of a vector x. Do not give a main function. Vector is length n, use C and CUDA to compute in parallel, allocate and free the GPU vectors and make the CPU - GPU memory transfers in the function. The size of blocks of threads and the number of blocks must be defined. Use next function name and parameters for the kernel __global__ void snrm2_kernel(int n, float *x, float *res) { where the result of computing the Ecludian norm of the vector x is returned in the pointer res, and the next function name and parameters for the function float chatblas_snrm2(int n, float *x) {. Include the next line at the beginning of the response #include \"chatblas_cuda.h\"."
  elseif ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "hip" 
...
function sscal()
  ...
  if ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "openmp" 
    prompt = "Only respond with code as plain text without code block syntax around it. Give me a function code that scales a vector x by a constant a, vector x is length n, use C and OpenMP to compute in parallel, use next function name and parameters void chatblas_sscal( int n, float a , float *x) {. Include the next line at the beginning #include \"chatblas_openmp.h\""
  elseif ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "cuda" 
    prompt = "Only respond with code as plain text without code block syntax around it. Give me a kernel and a function that calls the kernel only that scales a vector x by a constant a. Do not give a main function. Vector x is length n, use C and CUDA to compute in parallel, allocate and free the GPU vector and make the CPU - GPU memory transfers in the function. The size of blocks of threads and the number of blocks must be defined. Use next function name and parameters for the kernel __global__ void sscal_kernel( int n, float a , float *x ) { and the next function name and parameters for the function void chatblas_sscal( int n, float a, float *x) {. Include the next line at the beginning of the response #include \"chatblas_cuda.h\"."
  elseif ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "hip" 
...
function saxpy()
  ...
  if ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "openmp" 
    prompt = "Only respond with code as plain text without code block syntax around it. Give me a function code only that computes a multiplication of a vector x by a constant a and the result is added to a vector y. Vectors x and y are length n, use C and OpenMP to compute in parallel, use next function name and parameters void chatblas_saxpy(int n, float a, float *x, float *y). Include the next line at the beginning #include \"chatblas_openmp.h\""		
  elseif ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "cuda" 
...
function sasum()
  ...
  if ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "openmp" 
    prompt = "Only respond with code as plain text without code block syntax around it. Give me a function code only that returns the sum of the absolute values of the elements of a vector x. Vector x is length n, use C and OpenMP to compute in parallel include the next line in the code, use next function name and parameters float chatblas_sasum(int n, float *x). Include the next line at the beginning #include \"chatblas_openmp.h\""
  elseif ChatBLASPreferences.language == "c" && ChatBLASPreferences.model == "cuda" 
