...
@testset "Test Model" begin
    @test ChatBLAS.ChatBLASPreferences.model == "cuda"
end
...
@testset "All tests" begin
  ...
  @testset "ISAMAX" begin
    ...
    println("ISAMAX: ", ret, ", ", ret_ref)
    @test isapprox(ret, ret_ref, atol = 0.9)
  end
  ...
  @testset "SSWAP" begin
    ...
    @test isapprox(X, X_ref, atol = 0.9)
    @test isapprox(Y, Y_ref, atol = 0.9)
  end
  ...
  @testset "SDSDOT" begin
    function sdsdot(n::Int32, b::Float32, X::Vector{Float32}, Y::Vector{Float32})
      ...
      for i in 1:n
        tmp64 = tmp64 + X64[i] * Y64[i]
      end 
      b64::Float64 = 0.0
      b64 = convert(Float64, b)
      tmp64 = tmp64 + b64
      tmp::Float32 = 0.0
      tmp = convert(Float32, tmp64)
      return tmp 
    end
    n::Int32 = 100 
    b::Float32 = 3.0 
